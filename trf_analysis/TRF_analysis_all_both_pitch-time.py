...
trf_ori_both_pitch = np.zeros((len(subject_list),4,102,32))
trf_ori_both_time = np.zeros((len(subject_list),4,102,32))
trf_ori_A = np.zeros((len(subject_list),2,102,32))

trf_chimera_both = np.zeros((len(subject_list),6,102,32))
trf_chimera_both_pitch = np.zeros((len(subject_list),4,102,32))
trf_chimera_both_time = np.zeros((len(subject_list),4,102,32))
trf_chimera_A = np.zeros((len(subject_list),2,102,32))

ori_both_correlation = np.zeros((len(subject_list),66))
ori_both_pitch_correlation = np.zeros((len(subject_list),66))
ori_both_time_correlation = np.zeros((len(subject_list),66))
ori_A_correlation = np.zeros((len(subject_list),66))

chimera_both_correlation = np.zeros((len(subject_list),66))
chimera_both_pitch_correlation = np.zeros((len(subject_list),66))
chimera_both_time_correlation = np.zeros((len(subject_list),66))
...
for i, subject in enumerate(subject_list):
    ...
    trf_data = read_hdf5(data_root+'/'+subject+'_32chn_eeg_125hz_ltm_stm_both_nb_pitch_ioi_hp1_lp8_ICA_trf_data_-02_1_corr_all_crossval_best_reg_individual_pitch-time.hdf5')

    trf_ori_both[i] = trf_data['ori_both_weights']
    trf_ori_both_pitch[i] = trf_data['ori_bot_pitch_weights']
    trf_ori_both_time[i] = trf_data['ori_both_time_weights']
    trf_ori_A[i] =trf_data['ori_A_weights']

    trf_chimera_both[i] = trf_data['chimera_both_weights']
    trf_chimera_both_pitch[i] = trf_data['chimera_both_pitch_weights']
    trf_chimera_both_time[i] = trf_data['chimera_both_time_weights']
    trf_chimera_A[i] = trf_data['chimera_A_weights']
    
    ori_both_correlation[i] = trf_data['ori_both_correlation']
    ori_both_pitch_correlation[i] = trf_data['ori_both_pitch_correlation']
    ori_both_time_correlation[i] = trf_data['ori_both_time_correlation']
    ori_A_correlation[i] = trf_data['ori_A_correlation']
    
    chimera_both_correlation[i] = trf_data['chimera_both_correlation']
    chimera_both_pitch_correlation[i] = trf_data['chimera_both_pitch_correlation']
    chimera_both_time_correlation[i] = trf_data['chimera_both_time_correlation']
...
#    for chi_n in range(32):
    for chi_n in [1]:
        ...
        plt.tight_layout()
        plt.show()
        plt.savefig(figure_path+'zscore_reg_individual_both_'+regressors[reg]+'_'+channel_names[chi_n]+'.svg', dpi=dpi, format='svg')
...
prefer_df = pd.read_pickle('D://AMPLab/MusicExp/preference.pkl')
    ...
    for ci in ['original','chimera']:
        for ei in range(66):
            if ci == 'original':
                ...
                for mi in ['A','both','both_pitch','both_time']:
                    if mi == 'A':
                        ...
                        r = ori_both_pitch_correlation[si,ei]
                        g = set_num_ori[si,ei]
                    elif mi == 'both_time':
                        r = ori_both_time_correlation[si,ei]
                        g = set_num_ori[si,ei]
                ...
                for mi in ['A','both','both_pitch','both_time']:
                    if mi == 'A':
                        ...
                        r = chimera_both_pitch_correlation[si,ei]
                        g = set_num_chimera[si,ei]
                    elif mi == 'both_time':
                        r = chimera_both_time_correlation[si,ei]
                        g = set_num_chimera[si,ei]
                    
                    df = df._append({'subject':si, 'musician':ms, 'category':ci, 'group':g, 'model':mi, 'prefer':pr, 'r':r},ignore_index=True)

df.to_csv(exp_path+'expectation_effect_ltm_stm_both_nb_pitch_ioi_linear_model_zscore_reg_individual_pitch-time.csv')
...
pearsonr(mean_subject_group[(mean_subject_group['model'] == 'both_pitch')]['r'],mean_subject_group[(mean_subject_group['model'] == 'both_pitch')]['musician'])
pearsonr(mean_subject_group[(mean_subject_group['model'] == 'both_time')]['r'],mean_subject_group[(mean_subject_group['model'] == 'both_time')]['musician'])

# Plot delta_r correlation musicianship
# Correlation with fitted line
m_ori, b_ori = np.polyfit(mean_subject_group[(mean_subject_group['model']=="both")&(mean_subject_group['category']=="original")]['musician'], mean_subject_group[(mean_subject_group['model']=="both")&(mean_subject_group['category']=="original")]['r'], 1)
m_chimera, b_chimera = np.polyfit(mean_subject_group[(mean_subject_group['model']=="both")&(mean_subject_group['category']=="chimera")]['musician'], mean_subject_group[(mean_subject_group['model']=="both")&(mean_subject_group['category']=="chimera")]['r'], 1)

fig = plt.figure(dpi=300)
fig.set_size_inches(3.5,3)
plt.scatter(mean_subject_group[(mean_subject_group['model']=="both")&(mean_subject_group['category']=="original")]['musician'],mean_subject_group[(mean_subject_group['model']=="both")&(mean_subject_group['category']=="original")]['r'],c='C0',s=10, label='Original')
plt.plot(mean_subject_group[(mean_subject_group['model']=="both")&(mean_subject_group['category']=="original")]['musician'],m_ori*np.array(mean_subject_group[(mean_subject_group['model']=="both")&(mean_subject_group['category']=="original")]['musician'])+b_ori,c='C0')
plt.scatter(mean_subject_group[(mean_subject_group['model']=="both")&(mean_subject_group['category']=="chimera")]['musician'],mean_subject_group[(mean_subject_group['model']=="both")&(mean_subject_group['category']=="chimera")]['r'],marker='o', edgecolor='C0',facecolor='none', linewidths=1,s=10, label='Chimeric')
chimera_x = np.array(mean_subject_group[(mean_subject_group['model']=="both")&(mean_subject_group['category']=="chimera")]['musician'])
plt.plot([min(chimera_x),max(chimera_x)],m_chimera*np.array([min(chimera_x),max(chimera_x)])+b_chimera,c='C0',linestyle='--')
...
plt.gca().spines['top'].set_visible(False)
plt.gca().spines['right'].set_visible(False)
plt.savefig(figure_path+"r_both_musician_corr.svg",dpi=dpi,format='svg', bbox_inches='tight')

m_ori, b_ori = np.polyfit(mean_subject_group[(mean_subject_group['model']=="A")&(mean_subject_group['category']=="original")]['musician'], mean_subject_group[(mean_subject_group['model']=="A")&(mean_subject_group['category']=="original")]['r'], 1)
m_chimera, b_chimera = np.polyfit(mean_subject_group[(mean_subject_group['model']=="A")&(mean_subject_group['category']=="chimera")]['musician'], mean_subject_group[(mean_subject_group['model']=="A")&(mean_subject_group['category']=="chimera")]['r'], 1)

fig = plt.figure(dpi=300)
fig.set_size_inches(3.5,3)
plt.scatter(mean_subject_group[(mean_subject_group['model']=="A")&(mean_subject_group['category']=="original")]['musician'],mean_subject_group[(mean_subject_group['model']=="A")&(mean_subject_group['category']=="original")]['r'],c='C0',s=10, label='Original')
plt.plot(mean_subject_group[(mean_subject_group['model']=="A")&(mean_subject_group['category']=="original")]['musician'],m_ori*np.array(mean_subject_group[(mean_subject_group['model']=="A")&(mean_subject_group['category']=="original")]['musician'])+b_ori,c='C0')
plt.scatter(mean_subject_group[(mean_subject_group['model']=="A")&(mean_subject_group['category']=="chimera")]['musician'],mean_subject_group[(mean_subject_group['model']=="A")&(mean_subject_group['category']=="chimera")]['r'],marker='o', edgecolor='C0',facecolor='none', linewidths=1,s=10, label='Chimeric')
chimera_x = np.array(mean_subject_group[(mean_subject_group['model']=="A")&(mean_subject_group['category']=="chimera")]['musician'])
plt.plot([min(chimera_x),max(chimera_x)],m_chimera*np.array([min(chimera_x),max(chimera_x)])+b_chimera,c='C0',linestyle='--')
plt.ylabel("Prediction Accuracy \n A (r)")
plt.xlabel("Formal music training years")
plt.ylim(0.05,0.25)
...
plt.gca().spines['top'].set_visible(False)
plt.gca().spines['right'].set_visible(False)
plt.savefig(figure_path+"r_A_musician_corr.svg",dpi=dpi,format='svg', bbox_inches='tight')
...
plt.bar(br3[0], mean_ori_both_pitch.mean(), color='C1', width=barWidth, label='$AM_{pitch}$',zorder=3)
plt.bar(br3[1], mean_chimera_both_pitch.mean(), edgecolor='C1', fill=False, linewidth=2, width=barWidth,zorder=3)
plt.bar(br4[0], mean_ori_both_time.mean(), color='C2', width=barWidth, label='$AM_{time}$',zorder=3)
plt.bar(br4[1], mean_chimera_both_time.mean(), edgecolor='C2', fill=False, linewidth=2, width=barWidth,zorder=3)
plt.errorbar([br1[0],br1[1],br2[0],br2[1],br3[0],br3[1],br4[0],br4[1]], y=y, yerr=yerr,linestyle="", capsize=3.5, color="k",zorder=4,linewidth=1)
...
plt.gca().spines['top'].set_visible(False)
plt.gca().spines['right'].set_visible(False)
plt.savefig(figure_path+'model_prediction_accuracy_r_reg1_zscore_reg_individual_pitch-time.svg', dpi=dpi, format='svg', bbox_extra_artists=(lg,), bbox_inches='tight')
...
delta_r_df = pd.concat([delta_r_df, pd.DataFrame({"model":np.array(["both"]*len(mean_ori_A_group)),"category":["original"]*len(mean_ori_A_group), "random_eff":list(mean_ori_A_group),"musician":list(mean_ori_A_musician),
                                "delta_r":list(mean_ori_both-mean_ori_A)})])
delta_r_df = pd.concat([delta_r_df, pd.DataFrame({"model":np.array(["both_pitch"]*len(mean_ori_A_group)),"category":["original"]*len(mean_ori_A_group), "random_eff":list(mean_ori_A_group),"musician":list(mean_ori_A_musician),
                                "delta_r":list(mean_ori_both_pitch-mean_ori_A)})])
delta_r_df = pd.concat([delta_r_df, pd.DataFrame({"model":np.array(["both_time"]*len(mean_ori_A_group)),"category":["original"]*len(mean_ori_A_group), "random_eff":list(mean_ori_A_group),"musician":list(mean_ori_A_musician),
                                "delta_r":list(mean_ori_both_time-mean_ori_A)})])

delta_r_df = pd.concat([delta_r_df, pd.DataFrame({"model":np.array(["both"]*len(mean_chimera_A_group)),"category":["chimera"]*len(mean_chimera_A_group), "random_eff":list(mean_chimera_A_group),"musician":list(mean_chimera_A_musician),
                                "delta_r":list(mean_chimera_both-mean_chimera_A)})])
delta_r_df = pd.concat([delta_r_df, pd.DataFrame({"model":np.array(["both_pitch"]*len(mean_chimera_A_group)),"category":["chimera"]*len(mean_chimera_A_group), "random_eff":list(mean_chimera_A_group),"musician":list(mean_chimera_A_musician),
                                "delta_r":list(mean_chimera_both_pitch-mean_chimera_A)})])
delta_r_df = pd.concat([delta_r_df, pd.DataFrame({"model":np.array(["both_time"]*len(mean_chimera_A_group)),"category":["chimera"]*len(mean_chimera_A_group), "random_eff":list(mean_chimera_A_group),"musician":list(mean_chimera_A_musician),
...
pearsonr(delta_r_mean_subject_group[delta_r_mean_subject_group['model']=="both"]['musician'], delta_r_mean_subject_group[delta_r_mean_subject_group['model']=="both"]['delta_r'])
...
# ##### Only Both model
y=[mean_ori_both.mean(), mean_chimera_both.mean()]
yerr = [mean_ori_both.std()/np.sqrt(len(mean_ori_A_group)),
        mean_chimera_both.std()/np.sqrt(len(mean_ori_A_group)),]
...
fig = plt.figure(dpi=dpi)
fig.set_size_inches(3, 3)
plt.bar(br1, mean_ori_both.mean(), color='C0', width=barWidth, label='Original',zorder=3)
plt.bar(br2, mean_chimera_both.mean(), edgecolor='C0', linewidth=2, fill=False, width=barWidth, label='Chimeric',zorder=3)
plt.errorbar([br1, br2[0]], y=y, yerr=yerr,linestyle="", capsize=3.5, color="k",zorder=4,linewidth=1)
#plt.xlabel('Model')
plt.ylabel("Melodic Expectation Effect \n (Δr)")
#plt.ylim(-0, 0.045)
plt.ylim(-0.0025, 0.01)
plt.xlim(-0.5, 1)
plt.xticks([0.25], ['$AM$-A'])
for si in range(len(subject_list)):
    plt.plot([br1[0],br2[0]],[mean_ori_both_subject[si],mean_chimera_both_subject[si]], ".-", markersize=2, linewidth=0.5, c='grey',zorder=5,alpha=0.5)
...
plt.gca().spines['top'].set_visible(False)
plt.gca().spines['right'].set_visible(False)
plt.savefig(figure_path+'model_prediction_accuracy_delta_r_subject_only_both.svg', dpi=dpi, format='svg', bbox_extra_artists=(lg,), bbox_inches='tight')
